...
7. The directory processed stores all the denoised DAS data. 
...
10. The difference between the two "similar repositories" dasmrrcoh and dasmrrcoh-dataonly is that the latter is only used for storing the data (several GBs) and the former is only used for storing the scripts (less than 1 MB). 
